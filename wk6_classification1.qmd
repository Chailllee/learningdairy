...
To mitigate overfitting in decision trees, a common approach is to prune the tree, which simplifies the model by removing less significant branches(Friedman, J., 2009[^wk6_classification1-7];Quinlan, J. R. ,2014.[^wk6_classification1-8]). The effectiveness of this pruning process can be quantified using the Tree score formula:
...
Gini impurity is a metric used to measure the purity of a dataset, with smaller values indicating higher purity(Breiman, L., 2017[^wk6_classification1-4]).
...
$ccp_{\alpha}$ is a cost complexity parameter(Quinlan, J. R. ,2014.[^wk6_classification1-8]; Breiman, L., 2017[^wk6_classification1-4]) that balances the fit and complexity of the model during pruning.
...
[^wk6_classification1-4]:Breiman, L. (2017). Classification and regression trees. Routledge.

[^wk6_classification1-7]:Friedman, J. (2009). The elements of statistical learning: Data mining, inference, and prediction. (No Title).

[^wk6_classification1-8]:Quinlan, J. R. (2014). C4. 5: programs for machine learning. Elsevier.